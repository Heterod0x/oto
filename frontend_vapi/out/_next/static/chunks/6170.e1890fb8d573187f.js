...
`;
      var c = function (e, t, o, a) {
        var r,
          n = arguments.length,
          s = n < 3 ? t : null === a ? (a = Object.getOwnPropertyDescriptor(t, o)) : a;
        if ("object" == typeof Reflect && "function" == typeof Reflect.decorate)
          s = Reflect.decorate(e, t, o, a);
        else
          for (var i = e.length - 1; i >= 0; i--)
            (r = e[i]) && (s = (n < 3 ? r(s) : n > 3 ? r(t, o, s) : r(t, o)) || s);
    ...
    94061: (e, t, o) => {
      ...
      let g = {
          ...
          async fetchGasPrice() {
            ...
            try {
              ...
              {
                let e = (await p.x?.estimateGas({ chainNamespace: "solana" }))?.toString();
          ...
          async fetchSwapAllowance({
            ...
            let r = await T.T.fetchSwapAllowance({ tokenAddress: e, userAddress: t });
            if (r?.allowance && o && a) {
          ...
          async getMyTokensWithBalance(e) {
            ...
            if (!t || !o) return [];
            let a = (await T.T.getBalance(t, o.caipNetworkId, e)).balances.filter(
              (e) => "0" !== e.quantity.decimals,
            );
            return d.U.setTokenBalance(a, k.W.state.activeChain), this.mapBalancesToSwapTokens(a);
          },
          mapBalancesToSwapTokens: (e) =>
            e?.map((e) => ({
              ...e,
              address: e?.address ? e.address : k.W.getActiveNetworkTokenAddress(),
        ...
        m = {
          ...
          isInsufficientNetworkTokenForGas: (e, t) =>
            !!n.S.bigNumber(e).eq(0) || n.S.bigNumber(n.S.bigNumber(t || "0")).gt(e),
      ...
      let b = {
        ...
        y = {
          ...
          setSourceToken(e) {
            if (!e) {
              (f.sourceToken = e), (f.sourceTokenAmount = ""), (f.sourceTokenPriceInUSD = 0);
              return;
            }
          ...
          setToToken(e) {
            if (!e) {
              (f.toToken = e), (f.toTokenAmount = ""), (f.toTokenPriceInUSD = 0);
              return;
            }
            ...
            if (!f.initializing) {
              if (((f.initializing = !0), !f.initialized))
                try {
                  ...
                  (f.initialized = !1), A.P.showError("Failed to initialize swap"), P.I.goBack();
                }
              f.initializing = !1;
            }
          },
          async fetchTokens() {
            ...
            t &&
              ((f.networkTokenSymbol = t.symbol), I.setSourceToken(t), I.setSourceTokenAmount("1"));
          },
          async getTokenList() {
          ...
          async getAddressPrice(e) {
            ...
            let o = await T.T.fetchTokenPrice({ addresses: [e] }),
              ...
              s = parseFloat(
                (a.find((e) => e.symbol.toLowerCase() === n?.toLowerCase())?.price || 0).toString(),
              );
          ...
          async getNetworkTokenPrice() {
            let { networkAddress: e } = I.getParams(),
              t = await T.T.fetchTokenPrice({ addresses: [e] }).catch(
                () => (A.P.showError("Failed to fetch network token price"), { fungibles: [] }),
              ),
              o = t.fungibles?.[0],
          ...
          setBalances(e) {
            ...
            e.forEach((e) => {
              ...
              (f.myTokensWithBalance = e.filter((e) => e.address.startsWith(o.caipNetworkId))),
          ...
          async getInitialGasPrice() {
            ...
            if (k.W.state?.activeCaipNetwork?.chainNamespace === "solana")
              return (
                ...
                (f.gasPriceInUSD = n.S.multiply(e.standard, f.networkPrice).div(1e9).toNumber()),
                { gasPrice: BigInt(f.gasFee), gasPriceInUSD: Number(f.gasPriceInUSD) }
              );
            {
              let t = e.standard ?? "0",
                o = BigInt(t),
              ...
              return (f.gasFee = t), (f.gasPriceInUSD = r), { gasPrice: o, gasPriceInUSD: r };
            }
          },
          async swapTokens() {
            ...
            if ((a || I.setToTokenAmount(""), !o || !t || f.loadingPrices || !a)) return;
            f.loadingQuote = !0;
          ...
          async createAllowanceTransaction() {
            let { fromCaipAddress: e, sourceTokenAddress: t, toTokenAddress: o } = I.getParams();
            if (e && o) {
              if (!t) throw Error("createAllowanceTransaction - No source token address found.");
              try {
                let a = await T.T.generateApproveCalldata({ from: t, to: o, userAddress: e }),
                  r = {
                    data: a.tx.data,
          ...
          async createSwapTransaction() {
            let { networkAddress: e, fromCaipAddress: t, sourceTokenAmount: o } = I.getParams(),
              a = f.sourceToken,
              r = f.toToken;
            if (!t || !o || !a || !r) return;
              ...
              (f.transactionError = e?.shortMessage),
                ...
                v.E.sendEvent({
                  ...
                  properties: {
                    ...
                    isSmartAccount:
                      d.U.state.preferredAccountTypes?.eip155 === i.Vl.ACCOUNT_TYPES.SMART_ACCOUNT,
                  },
                });
            }
          },
          async sendTransactionForSwap(e) {
            if (!e) return;
            let { fromAddress: t, toTokenAmount: o, isAuthConnector: a } = I.getParams();
            f.loadingTransaction = !0;
            let r = `Swapping ${f.sourceToken?.symbol} to ${n.S.formatNumberToLocalString(o, 3)} ${f.toToken?.symbol}`,
              s = `Swapped ${f.sourceToken?.symbol} to ${n.S.formatNumberToLocalString(o, 3)} ${f.toToken?.symbol}`;
            a
              ? P.I.pushTransactionStack({
                  onSuccess() {
