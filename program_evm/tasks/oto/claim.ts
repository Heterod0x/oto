...
task("oto:claim", "Claim tokens for user points")
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    try {
      ...
      const balanceBefore = await oto.read.balanceOf([walletClient.account.address]);
      const decimals = await oto.read.decimals();
      console.log(`Current token balance: ${balanceBefore / (10n ** BigInt(decimals))} OTO`);
      ...
      const balanceAfter = await oto.read.balanceOf([walletClient.account.address]);
      console.log(`New token balance: ${balanceAfter / (10n ** BigInt(decimals))} OTO`);
      console.log(`Tokens received: ${(balanceAfter - balanceBefore) / (10n ** BigInt(decimals))} OTO`);
      ...
      if (events.length > 0) {
        ...
        console.log(`- Claim Amount: ${latestEvent.args.claimAmount!.toString()}`);
      }
